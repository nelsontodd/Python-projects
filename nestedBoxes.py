...
def sub_sets (a, b, idx, unsortednestedboxes): #Finds all sub sets of the input and adds them to the list if they are a nestedbox
  if (idx == len(a)):
          if checkBoxes(b) == True:
...
def main():
    listBoxes = []
    with open('Boxes.txt') as Boxes: #open the boxes.txt file
        Inputtext = Boxes.read().splitlines()
        map(int, Inputtext[0])
    ...
    for s in range(len(unsortednestedboxes)): #print the output
        if len(unsortednestedboxes[s]) == longest:
